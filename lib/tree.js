...
function createTree(delNode, x, y, width, height, scale, expNode) {
  ...
  // ************** Generate the tree diagram	 *****************
    ...
    node = document.getElementById("tree-canvas")
    svg = d3.select(node).append("svg")
    	.attr("width", "100vw")//width + margin.right + margin.left)
...
//      .style("background", "#fff")
  ...
  let expandButton = nodeContainer.filter(d => d.children && d.children.filter(c => d.id == c.ancestors[0]).length != d.children.length)
    .append("xhtml:div")
...
*/    
  ...
  let camera = nodeContainer.filter(d => d.photo)
    .append("xhtml:img")
    .attr("src", "/camera.png")
    .style("background", "#fff")
    .style("padding", "0.2em")
    .style("border-radius", "50%")
    .style("box-shadow", "0 2px 5px rgba(120, 120, 120, 0.4)")
    .style("position", "absolute")
    .style("bottom", "48px")
    .style("width", "23px")
    .style("height", "23px")
