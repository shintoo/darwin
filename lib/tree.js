...
//      .style("background", "#fff")
  ...
  let nodeButtons = nodeContainer
    .append("xhtml:div")
    .attr("class", "node-buttons")

  let nodeDelButton = nodeButtons.filter(d => !d.children)
    .append("xhtml:img")
    .attr("src", "/x.png")
    .attr("class", "node-button")
    .on("click", d => d.children ? null : (console.log("click on node"), deleteNode(d.id)))
    
  let nodeInfoButton = nodeButtons
    .append("xhtml:img")
    .attr("src", "/info.png")
    .attr("class", "node-button")
    .on("click", _ => console.log("clicked info button"))
  ...
  let nodeText = nodeEnter.append("text")
    .attr("y", d => d.children ? "-70px" : "40px")
      ...
      .text(d => {
        console.log("label options: ", d.common_name, d.sci_name)
