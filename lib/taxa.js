...
export default function getTaxa(query, rank, parent, id, page, per_page) {
  ...
  console.log("getTaxa call with args: ", query, rank, parent, id, page, per_page)
  if (id) {
      url += "/" + id
  }
  ...
  return fetch(url)
    ...
    .then(data => {
      if (!data.results)
        return [-1]
      return data.results.map(simplifyTaxon)
    })
}

export function simplifyTaxon(taxon) {
  return {
    id: taxon.id,
    ...
    ancestors: taxon.ancestor_ids.reverse(),
    naming: {
      common_name: typeof(taxon.preferred_common_name) !== "undefined" ? taxon.preferred_common_name : null,
      rank: taxon.rank,
      taxon: taxon.name
    },
...
// TODO remove the need for this, seriously
