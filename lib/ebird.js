...
const options = {
  mode: 'cors',
  headers: new Headers({ "x-eBirdApiToken": process.env.EBIRD_TOKEN })
}

export default async function getEBirdChecklist(checklistId) {
  ...
  const species = await getSpeciesFromEBirdCodes(speciesCodes)
  const location = await getLocationName(data.locId)
...
async function getSpeciesFromEBirdCodes(speciesCodes) {
